...
class feature_extraction:
    ...
    def fRankOp(self, transformer, data_set, query_sample, label_Wk,n):
        data_set_trans = transformer(data_set)
        m = data_set_trans.shape[0]
        K = np.zeros((1, m))
        distance = []
        for k in range(m):
            dist = np.linalg.norm(data_set_trans .lioc[k, :] - query_sample)  # Euclidean distance
            distance.append(dist)
        distance_df = pd.DataFrame(distance, index=list(range(m)))
        distance_df.sort()
        knn = KNeighborsClassifier(n_neighbors=n)  # set neighbors to 3
        knn.fit(data_set_trans, label_Wk)
        pred = knn.predict(data_set_trans [distance_df.index])
        for j in range(pred):
    ...
    def fOp(self, data_set, transformer, classifier):
        '''
        :param data_set:
        :param transformer: transform x into x~
        :param classifier:
        :param label_Wk:
        :return:
        '''
        m = transformer(data_set).shape[0]
        K = np.zeros((1, m))
        for k in range(m):
            if classifier(data_set.lioc[k,:]) == classifier(transformer(data_set).lioc[k,:]):
                K[k] = 1
        return K

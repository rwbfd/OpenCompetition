...
flags.DEFINE_string("model_dir", "gs://checkpoints_for_pretrained_lm/model_dir_bert", None)
...
#flags.DEFINE_bool("use_text_b", False, None)
...
flags.DEFINE_string(
    "init_checkpoint",
    #"gs://ccf_cp/ccf/pretrain_model/ernie1.0/bert_model.ckpt",
    None,
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("loss"):
        ...
        if FLAGS.use_multi_loss:
            ...
            # focal loss
            if FLAGS.use_label_smoothing:
                per_example_fl_loss = -tf.reduce_sum(one_hot_labels_ls * (1 - probabilities) ** FLAGS.fl_loss_gama * log_probs, -1)
            else:
                per_example_fl_loss = -tf.reduce_sum(one_hot_labels * (1 - probabilities) ** FLAGS.fl_loss_gama * log_probs, -1)
            fl_loss = tf.reduce_mean(per_example_fl_loss)

            tf.logging.info("**** fl loss ****: " + str(fl_loss))

            # hige loss
            per_example_hg_loss = tf.losses.hinge_loss(labels=one_hot_labels, logits=logits)
            hg_loss = tf.reduce_mean(per_example_hg_loss)

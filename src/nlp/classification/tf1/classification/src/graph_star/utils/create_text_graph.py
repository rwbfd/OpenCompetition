...
def remove_words(dataset):
    ...
    print('enter remove words------------------------------\n')
    stop_words = set(stopwords.words('english'))
    print("loading stop_words------------------------------\n")

    doc_content_list = []
    tc_corpus = os.path.join(cur_dir, '../data/TC/corpus/' + dataset + '.txt')
    f = open(tc_corpus, 'rb')
    print("reading file:%s" % tc_corpus)
    ...
    f = open(os.path.join(cur_dir, '../data/TC/corpus/' + dataset + '.clean.txt'), 'w')
    ...
    f = open(os.path.join(cur_dir, '../data/TC/corpus/' + dataset + '.clean.txt'), 'r')
    lines = f.readlines()
    for line in lines:
        line = line.strip()
        temp = line.split()
...
def loadWord2Vec(dataset, use_pretrain_vec=True, vocab=None, emb_sz=200):
    ...
    if use_pretrain_vec:
        if dataset == '20ng':
            filename = os.path.join(cur_dir, '../data/TC/' + dataset + '_word_vectors_0.txt')
        else:
            filename = os.path.join(cur_dir, '../data/TC/' + dataset + '_word_vectors.txt')
            print("loadWord2Vec!")
            print(filename)
        vocab = []
        embd = []
        word_vector_map = {}
        print(f'word_vector_filename: {filename}')
        with open(filename, 'r') as file:
            for line in tqdm(file.readlines()):
                row = line.strip().split(' ')
                # print(f'row: {row}')
                #print(f'word: {row[0]}; code: {type(row[0])}')
                # print(f'word: {row[0]}; code: {type(row[0])}, {chardet.detect(row[0])}')
                if(len(row) > 2):
                    vocab.append(row[0])
                    vector = row[1:]
                    length = len(vector)
        ...
        print(word_vector_map.keys())
        print('Loaded Word Vectors!')
    else:
        embd = []
        word_vector_map = {}
        word_vectors = np.random.uniform(-0.01, 0.01, (len(vocab), emb_sz))
        for word, vec in tqdm(zip(vocab, word_vectors)):
...
def split_train_test(dataset):
    ...
    f = open(os.path.join(cur_dir, '../data/TC/' + dataset + '.txt'), 'r')
    lines = f.readlines()
    for line in lines:
...
def write_train_test_ids(doc_name_list, doc_train_list, doc_test_list, dataset):
    ...
    f = open(os.path.join(cur_dir, '../data/TC/' + dataset + '.train.index'), 'w')
    ...
    f = open(os.path.join(cur_dir, '../data/TC/' + dataset + '.test.index'), 'w')
    f.write(test_ids_str)
    f.close()

    ids = train_ids + test_ids
    print("traind_ids + test_ids is %d" % len(ids))
...
def create_shuffle_file(dataset, ids, doc_name_list, doc_content_list):
    ...
    f = open(os.path.join(cur_dir, '../data/TC/' + dataset + '_shuffle.txt'), 'w')
    f.write(shuffle_doc_name_str)
    f.close()

    f = open(os.path.join(cur_dir, '../data/TC/corpus/' + dataset + '_shuffle.txt'), 'w')
...
def build_vocab(dataset, shuffle_doc_words_list):
    ...
    f = open(os.path.join(cur_dir, '../data/TC/corpus/' + dataset + '_vocab.txt'), 'w')
    f.write(vocab_str)
    f.close()
...
def get_label_list(shuffle_doc_name_list, dataset):
    ...
    label_list_str = '\n'.join(label_list)
    f = open(os.path.join(cur_dir, '../data/TC/corpus/' + dataset + '_labels.txt'), 'w')
...
def split_train_val(dataset, train_ids, shuffle_doc_name_list):
    ...
    f = open(os.path.join(cur_dir, '../data/TC/' + dataset + '.real_train.name'), 'w')

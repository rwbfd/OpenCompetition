...
dataset = 'aclImdb'
use_word_feat = False  # whether to use pretrained word vector, default should be true
max_length = 172 #3072  # max length for the doc, 3072 is longer than the maxium length of mr, R8, R52, Ohsumed, but truncate for 20ng
word_embdz_size = 100#200  # only valid when use_word_feat is False
...
def main():
    ...
    if use_repeat_word:
        ...
        print("get_all_nodes_in_all_graph done...")
        graph_labels = get_graph_labels(shuffle_doc_name_list, shuffle_doc_words_list, label_list)
        print("get_graph_labels done...")
        max_num_nodes = get_max_num_node(shuffle_doc_words_list, max_length, use_repeat_word)
        print('get_max_num_node done... do not use repeat word')
        print('max num nodes is ' + str(max_num_nodes))
        print('total num of nodes is ' + str(len(node_attrs)))

...
def classification_loss(
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if FLAGS.use_multi_loss:
            ...
            if FLAGS.use_label_smoothing:
                tf.logging.info("****  label smoothing ****")
                per_example_fl_loss = -tf.reduce_sum(one_hot_target_ls * (1 - probabilities) ** FLAGS.fl_loss_gama * tf.nn.log_softmax(logits), -1)
            else:
                per_example_fl_loss = -tf.reduce_sum(one_hot_target * (1 - probabilities) ** FLAGS.fl_loss_gama * tf.nn.log_softmax(logits), -1)

            tf.logging.info("**** fl loss ****: " + str(per_example_fl_loss))

            # hige loss
            per_example_hg_loss = tf.losses.hinge_loss(labels=one_hot_target, logits=logits)
            tf.logging.info("**** hg loss ****: " + str(per_example_hg_loss))

            loss = FLAGS.ce_loss_weight * loss + FLAGS.fl_loss_weight * per_example_fl_loss + FLAGS.hg_loss_weight * per_example_hg_loss

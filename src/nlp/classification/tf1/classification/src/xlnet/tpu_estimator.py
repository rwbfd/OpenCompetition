...
def _increase_eval_step_op(iterations_per_loop):
    ...
    # Estimator evaluate increases 1 by default. So, we increase the
    # difference.
...
class TPUEstimatorSpec(
        model_fn_lib._TPUEstimatorSpec):  # pylint: disable=protected-access
    ...
    def as_estimator_spec(self):
        ...
        if tensor_tracer.TensorTracer.is_enabled():
            ...
            tracing_call_ret = _OutfeedHostCall.create_cpu_hostcall(
                tracing_calls)
...
class TPUInfeedOutfeedSessionHook(session_run_hook.SessionRunHook):
    ...
    def _run_infeed(self, queue_ctx, session):
        ...
        with self._rendezvous.catch_errors(source='infeed', session=session):
            if self._run_infeed_loop_on_coordinator:
                for count, steps in enumerate(
                        queue_ctx.read_iteration_counts()):
                    for i in xrange(steps):
                        logging.debug(
                            'Infeed enqueue for iteration (%d, %d)', count, i)
    ...
    def _run_outfeed(self, queue_ctx, session):
        ...
        with self._rendezvous.catch_errors(source='outfeed', session=session):
            for count, steps in enumerate(queue_ctx.read_iteration_counts()):
                for i in xrange(steps):
                    logging.debug(
                        'Outfeed dequeue for iteration (%d, %d)', count, i)
    ...
    def after_create_session(self, session, coord):
        ...
        if os.environ.get('TPU_SPLIT_COMPILE_AND_EXECUTE', '') == '1':
            ...
            self._assertCompilationSucceeded(
                session.run(self._tpu_compile_op), coord)
    ...
    def after_run(self, run_context, run_values):
        ...
        if _StopSignals.should_stop(scalar_stopping_signal):
            ...
            raise errors.OutOfRangeError(
                None, None, 'Stopped by stopping signal.')
        ...
        with ops.device(device):
            ...
            for _ in range(num_replicas_per_host):
                ...
                with ops.control_dependencies(control_deps):
                    ...
                    # This avoids inconsistent state among different model
                    # replcias.
                    if cached_signals:
...
class _InputPipeline(object):
    ...
    class InputsStructureRecorder(object):
        ...
        def __init__(self, input_partition_dims=None):
            ...
            if input_partition_dims:
                ...
                assert len(
                    input_partition_dims) <= 2, 'must have 1 or 2 elements.'
        ...
        def _flatten_input_dims(
            ...
            if feature_dims_names:
                # We need a fixed ordering for matching the tensors in
                # features.
            ...
            if label_dims_names:
                ...
                flattened_input_dims.extend(
                    [label_dims] * num_tensors_in_label)
    ...
    def generate_infeed_enqueue_ops_and_dequeue_fn(self):
        ...
        def dequeue_fn():
            ...
            # The unflatten process uses the structure information recorded
            # above.
            ...
            for host_id in range(num_hosts):
                ...
                with ops.device(host_device):
                    with ops.name_scope('input_pipeline_task%d' % (host_id)):
                        ...
                        # Infeed_queue_getter must be called after
                        # enqueue_ops_fn is called.
            ...
            for host_id in range(num_hosts):
                ...
                with ops.device(host_device):
                    with ops.name_scope('input_pipeline_task%d' % (host_id)):
                        ...
                        if dataset_initializer:
                            all_dataset_initializers.append(
                                dataset_initializer)
                            run_infeed_loop_on_coordinator = False
                            wrap_fn = (
                                _wrap_computation_in_while_loop
...
class _ModelFnWrapper(object):
    ...
    def convert_to_single_tpu_eval_step(self, dequeue_fn):
        ...
        def eval_step(total_loss, *cache):
            ...
            tpu_estimator_spec = self._call_model_fn(
                features, labels, cache=cache)
            if not isinstance(
                    tpu_estimator_spec,
                    model_fn_lib._TPUEstimatorSpec):  # pylint: disable=protected-access
                raise RuntimeError(
                    'estimator_spec used by TPU evaluation must have type'
                    '`TPUEstimatorSpec`. Got {}'.format(
                        type(tpu_estimator_spec)))
    ...
    def convert_to_single_tpu_predict_step(self, dequeue_fn):
        ...
        def predict_step(unused_scalar_stopping_signal):
            ...
            to_record['predictions'] = [
                identity_fn, tpu_estimator_spec.predictions]
    ...
    def _call_model_fn(
        ...
        self._validate_model_features_and_labels(
            features, labels, is_export_mode)
        ...
        if (running_on_cpu and isinstance(estimator_spec,
                                          model_fn_lib._TPUEstimatorSpec)):  # pylint: disable=protected-access
    ...
    def validate(host_calls):
        ...
        for name, host_call in host_calls.items():
            ...
            if isinstance(host_call[1], (tuple, list)):
                ...
                # wrapped_hostcall_with_global_step uses varargs, so we allow
                # that.
        ...
        with ops.device(self._ctx.tpu_host_placement_function(replica_id=0)):
            for name in self._names:
                ...
                for i, item in enumerate(dequeue_ops):
                    if dequeue_ops[i][0].shape.ndims == 0:
                        raise RuntimeError(
                            ...
                            'dimension, but got scalar {}'.format(
                                dequeue_ops[i][0]))
                ...
                if self._tensor_keys[name] is not None:
                    ...
                    dequeue_ops = dict(
                        zip(self._tensor_keys[name], dequeue_ops))
                    try:
    ...
    *should* raise an end-of-input exception (`OutOfRangeError` or
    ...
    def __init__(self,
        ...
        if params is not None and any(
                k in params for k in _RESERVED_PARAMS_KEYS):
            raise ValueError(
        ...
        estimator_lib._verify_model_fn_args(
            model_fn, params)  # pylint: disable=protected-access
    ...
    def _call_model_fn(self, features, labels, mode, config):
        if mode == _REWRITE_FOR_INFERENCE_MODE:
            return self._call_model_fn_for_inference(
    ...
    def _call_model_fn_for_inference(self, features, labels, mode, config):
        ...
        def computation():
            ...
            # so that building the graph will happen under
        ...
        with self._ctx.with_mode(model_fn_lib.ModeKeys.EVAL) as ctx:
            if ctx.is_running_on_cpu():
                return super(TPUEstimator,
                             self)._convert_eval_steps_to_hooks(steps)
    ...
    def _augment_model_fn(
        ...
        def _model_fn(features, labels, mode, config, params):
            ...
            with self._ctx.with_mode(mode) as ctx:
                ...
                # TPUEstimator._call_input_fn passes `input_fn` as features to
                # here.
                ...
                for enqueue_op in enqueue_ops:
                    if isinstance(enqueue_op, list):
                        graph.get_collection_ref(
                            _TPU_ENQUEUE_OPS).extend(enqueue_op)
                    else:
                ...
                if mode == model_fn_lib.ModeKeys.TRAIN:
                    ...
                    if shutdown_mode:
                        if shutdown_mode == 'shutdown_worker':
                            ...
                            finalizer_hooks = [
                                session_support.RestartComputation(
                                    timeout_ms=60 * 1000), ]
                        else:
                            raise ValueError(
                    ...
                    with ops.control_dependencies([loss]):
                        global_step = array_ops.identity(
                ...
                if mode == model_fn_lib.ModeKeys.EVAL:
                    ...
                    mean_loss = math_ops.div(total_loss, math_ops.cast(
                        iterations_per_loop_var, dtype=total_loss.dtype))
...
def _eval_on_tpu_system(ctx, model_fn_wrapper, dequeue_fn):
    ...
    def multi_tpu_eval_steps_on_single_shard():
        ...
        if model_fn_wrapper._eval_cache_fn is not None:
            ...
            loop_vars += model_fn_wrapper._eval_cache_fn(
...
def _train_on_tpu_system(ctx, model_fn_wrapper, dequeue_fn):
    ...
    def multi_tpu_train_steps_on_single_shard():
        ...
        if model_fn_wrapper._train_cache_fn is not None:
            batch_size = ctx.global_batch_size
            num_shards = ctx._config._tpu_config.num_shards
            loop_vars += model_fn_wrapper._train_cache_fn(
                batch_size // num_shards)
...
class _CapturingContext(control_flow_ops.ControlFlowContext):
    ...
    def __exit__(self, _, __, ___):  # pylint: disable=invalid-name
        self._g._set_control_flow_context(
            self._old)  # pylint: disable=protected-access
    ...
    def __init__(self,
        ...
        if num_invocations_per_step == 1:
            ...
            def _set_mask(data_dict):
                ...
                signals['padding_mask'] = array_ops.ones_like(
        ...
        dataset = user_provided_dataset.concatenate(
            final_batch_dataset).prefetch(2)
...
class _StopSignals(object):
    ...
    def should_stop(scalar_stopping_signal):
        ...
        if isinstance(scalar_stopping_signal, ops.Tensor):
            ...
            # way to express the bool check whether scalar_stopping_signal is
            # True.
...
class _PaddingSignals(object):
    ...
    def _find_any_tensor(batch_features):
        tensors = [x for x in nest.flatten(
            batch_features) if isinstance(x, ops.Tensor)]
        if not tensors:

...
class CategoryEncoders:
    ...
    def target_encoder(self,X,y,cols=None):
        encoder = TargetEncoder(cols=cols).fit(X,y)
        return encoder.transform(X)

    def catboost_encoder(self,X,y,cols=None):
        encoder = CatBoostEncoder(cols=cols).fit(X,y)
        return encoder.transform(X)

    def james_stein_encoder(self,X,y,cols=None):
        encoder = JamesSteinEncoder(cols=cols).fit(X,y)
        return encoder.transform(X)

    def leaveone_encoder(self,X,y,cols=None):
        encoder = LeaveOneOutEncoder(cols=cols).fit(X,y)
        return encoder.transform(X)

    def mestimate_encoder(self,X,y,cols=None):
        encoder = MEstimateEncoder(cols=cols).fit(X,y)
        return encoder.transform(X)

    def weight_of_evidence_encoder(self,X,y,cols=None):
        encoder = WOEEncoder(cols=cols).fit(X,y)
        return encoder.transform(X)
...
if __name__ == '__main__':
    ...
    print(numeric_dataset[['CHAS','RAD']])
    print(X[['CHAS','RAD']])

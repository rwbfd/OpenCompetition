...
class data_preprossing:
    def recognize_feature(self, data, label_encoder=False):
        sparse_features = []
        dense_features = []
    ...
    def hash_encoding(self, data, sparse_features):
        return


    def one_hot_for_sparse(self, data, sparse_features):
    ...
    def scalar_for_dense(self, data, dense_features):
    ...
    def get_feature(self,data):
        data, sparse_features, dense_features = self.recognize_feature(data)
        ...
        return linear_feature_columns,dnn_feature_columns,fixlen_feature_names
...
if __name__ == '__main__':
    ...
    fixlen_feature_names = data_preprossing().get_feature()[2]
    linear_feature_columns = data_preprossing().get_feature()[0]
    dnn_feature_columns = data_preprossing().get_feature()[1]
    train_model_input = [train[name] for name in fixlen_feature_names]

    test_model_input = [test[name] for name in fixlen_feature_names]

    # 4.Define Model,train,predict and evaluate
    # model = DeepFM(linear_feature_columns, dnn_feature_columns, embedding_size=3, dnn_hidden_units=(2048, 1024, 100),
